...
{
    word32 maxOutputLen = (pkcsBlockLen > 10U) ? (pkcsBlockLen - 10U) : 0U;
    word32 invalid = 0U;
    word32 i = 1U;
    word32 outputLen;
    ...
    if (pkcsBlock[0] != 0x0U) { /* skip past zero */
        invalid = 1U;
    }
    pkcsBlock++; pkcsBlockLen--;

    /* Require block type padValue */
    invalid = (word32)((pkcsBlock[0] != padValue) || (invalid != 0U));
    ...
    if (!(i==pkcsBlockLen || pkcsBlock[i-1U]==0U)) {
        WOLFSSL_MSG("RsaUnPad error, bad formatting");
        return RSA_PAD_E;
    }

    outputLen = pkcsBlockLen - i;
    invalid = (word32)((outputLen > maxOutputLen) || (invalid != 0U));
                          ...
                          word32* outLen, int type, RsaKey* key, WC_RNG* rng)
{
    ...
    if (mp_read_unsigned_bin(&tmp, (byte*)in, (int)inLen) != MP_OKAY)
        ERROR_OUT(MP_READ_E);
    ...
    keyLen = (word32)wc_RsaEncryptSize(key);
    if (keyLen > *outLen) {
        ERROR_OUT(RSA_BUFFER_E);
    }

    len = (word32)mp_unsigned_bin_size(&tmp);
